...
type GoExpectSsh struct {
	C *ssh.Client
}


func (s GoExpectSsh) Close() {
	s.C.Close()
}


func (s *GoExpectSsh) Open(h *app.Host) (error) {
...
func GetConn(h *app.Host) (app.Connector, error) {
	...
	conn, err := h.GetConnection("goexpect_ssh")
	if err == nil {
		return conn, nil
	}

	cc = app.Connector(&GoExpectSsh{})
	err = cc.Open(h)
	...
	h.SetConnection("goexpect_ssh", cc)
	return cc, nil
